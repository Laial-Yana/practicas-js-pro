//*Vamos a seguir el patron de diseño creacional Singleton, que va a hacer que la clase
//*se pueda instanciar solamente una vez, asi podemos conseguir siempre ads distintos y no repetidos
export interface Ad {
  imageUrl: string;
  title: string;
  body: string;
...
//Singleton tiene una estructura
export default class Ads {
  // intancia privatada estatica
  ...
  static getInstance() {
    //va a chequear que tenemos una instancia y si no la tenemos la creamos
    if (!Ads.instance) {
  ...
  //inicializar Ad[]
  private initAds() {
    //hacemos una copia de All_ads con un spread operator
    this.ads = [...ALL_ADS];
  }
  //Ads tiene que exponer una forma de dar los ads a quien se lo pida, entonces creamos un metodo
  getAd() {
    //si se terminaron de mostrar todos los ads que se inicien de nuevo
